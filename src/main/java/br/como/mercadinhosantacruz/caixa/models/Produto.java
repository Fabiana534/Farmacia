 ...
 */
package br.como.mercadinhosantacruz.caixa.models;
...
public class Produto {
    ...
    public void setPreco(float preco) {
     ...
     public void salvar(JdbcTemplate jdbc){
         if(id > 0){
         jdbc.update("UPDATE produtos SET nome = ?,  " + "preco = ?, qtd = ?" + "WHERE id = ?;", (ps) -> {
         ps.setString(1, nome);
         ps.setFloat(2, preco);
         ps.setInt(3, qtd);
         ps.setInt(4, id);
         ...
         jdbc.update("insert into produtos (nome, preco, qtd) " + "values (?,?,?)", (ps) -> {
         ps.setString(1, nome);
         ps.setFloat(2, preco);
         ps.setInt(3, qtd);
         }); 
     ...
     public static List<Produto> listar (JdbcTemplate jdbc){
        ...
        do{
           Produto p = new Produto (rs.getString ("nome"), rs.getInt("qtd"), rs.getFloat("preco"));
           p.id = rs.getInt("id");
           produtos.add(p);
        } while(rs.next());
     });
